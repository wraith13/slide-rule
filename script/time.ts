...
export const applyHumanCalendar = (json: Type.ConstantTable, _path?: string): Type.ConstantTable =>
{
    ...
    const getBCELabel = (years: number) =>
        Locale.label("NNN BCE", text => text.replace("NNN", Calculation.getNamedNumberLabel(years)));
    const getBeforeBPCEValue = (years: number): number =>
        parseRelativeUniverseEpoch(`${1950 -years} years ago`);
    const getAfterBPCEValue = (years: number): number =>
