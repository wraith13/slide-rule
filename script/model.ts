...
const anchorHumanEpochTime = new Date(config.model.time.anchor.humanEpoch).getTime();
export const humanEpochToUniverseEpoch = (humanEpoch: Date): number =>
    (humanEpoch.getTime() -anchorHumanEpochTime) /1000 +config.model.time.anchor.universeEpoch;
export const universeEpochToHumanEpoch = (universeEpoch: number): Date =>
{
    try
    {
        return new Date((universeEpoch -config.model.time.anchor.universeEpoch) /1000 +anchorHumanEpochTime);
    }
    catch(e)
    {
...
{
    const currentUniverseEpoch = config.model.time.anchor.universeEpoch;
...
{
    const now = config.model.time.anchor.universeEpoch;
    ...
    if (null !== match)
    {
        const value = Number.System.parseFloat(match[2]);
        const unit = match[3];
        switch(unit)
        {
        case "second":
